...
#define DPORT_PRO_GPIO_INTERRUPT_MAP_REG (*(volatile uint32_t*)0x3FF0015C) // interrupt matrix register for GPIO interrupts (PRO CPU / core 0)
...
static void IRAM_ATTR gpio_isr(void *arg) {
    ...
    GPIO_STATUS_INT_W1TC = (1 << BUTTON_PIN); // clear interrupt status for button pin
}
...
void app_main(void)
{
    ...
    GPIO_PIN4_REG = (GPIO_PIN4_REG & ~(0x1F << 13)) | (1 << 15); // clear bits 13-17 and sets bit 2 as 1: PRO CPU interrupt enable

    DPORT_PRO_GPIO_INTERRUPT_MAP_REG = 10; // set CPU interrupt. 10: Peripheral, Edge-Triggered, priority 1(low)

    xt_set_interrupt_handler(10, gpio_isr, NULL); // set interrupt handler for GPIO interrupt

    xt_ints_on(1 << 10); // enable interrupt for GPIO
